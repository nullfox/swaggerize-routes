...
function buildroutes(options) {
    var schemas, api, routes, handlers, validator;
    ...
    Object.keys(api.paths).forEach(function (path) {
        ...
        utils.verbs.forEach(function (verb) {
            var route, pathnames, operation, responses, response, validators;
            ...
            if (operation.responses.default && operation.responses.default.schema) {
                ...
                response = utils.resolveRef(schemas, schema.items ? schema.items.$ref : schema.$ref);

                Object.keys(response.properties).forEach(function (key) {
                    if (response.required && response.required.indexOf(key) !== -1) {
                        response.properties[key].required = true;
                    }
                });

                route.response = validator.makeAllResponse(
                    response.properties,
                    route
                );
            }
